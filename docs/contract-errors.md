...
the raw `#[repr(u32)]` values. Variants marked *reserved* are still part of the
...
| `4` | `InvalidTimeRange` | *Reserved* — an earlier name for the `end_time <= start_time` check; the current `create_stream` path reports `InvalidDuration` (code `27`) instead. |
...
| `18`–`21` | *(unassigned)* | Reserved numeric space between the original and later error additions. |
...
| `1005` | `CalculationOverflow` | *Reserved* — no current code path throws it (arithmetic errors surface as `ArithmeticOverflow`). |
...
| `2` | `StreamNotFound` | *Reserved* — formerly "invalid stream ID"; the current `state::load` reports `NotInitialized` for an inaccessible stream instance instead. |
...
| `7` | `InsufficientDeposit` | *Reserved* — deposit sufficiency is enforced by `DripFactory` at creation; no current `DripStream` code path throws it. |
...
| `17` | `OperatorAlreadySet` | *Reserved* — `set_operator` now atomically replaces an existing operator rather than rejecting; no current code path throws it. |
...
| Code | Name | Fires when |
|------|------|------------|
| `1` | `InvalidAmount` | `deposit`/`withdraw` is given `amount <= 0`, `initialize` is given `max_limit <= 0`, or `set_operator_withdraw_limit` is given `new_limit <= 0`. |
| `2` | `ArithmeticOverflow` | Integer overflow computing the expected vault balance after a `deposit`/`withdraw`. |
| `3` | `LimitExceeded` | A deposit would push the vault balance past `max_limit`; an operator withdraws more than `operator_withdraw_limit`; or `set_limit` tries to set a final balance below the current balance. |
...
| `5` | `ContractPaused` | `deposit`, `withdraw`, or `set_limit` is called while the vault is under an emergency pause. |
...
| `13` | `DepositTransferFailed` | The token transfer into the vault did not move exactly the expected amount for `deposit` (or for the balance check after `withdraw`). |
| `14` | `WithdrawTransferFailed` | The token transfer out of the vault did not move exactly the expected amount for `withdraw`. |